...
Test.prototype.getNextQuestion = function () {
  ...
  var lastAnswered = this.studentResponses[this.studentResponses.length - 1].answeredCorrectly;

  var question = this.findClosestDifficulty(ability, lastAnswered);
  if (!question)
  {
    question = this.findClosestDifficulty(ability, !lastAnswered);
    if (!question) {
      return false;
    }
    return question;
